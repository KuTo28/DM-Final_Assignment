...
```{r asoc rules}
tmp <- df %>% mutate(id = seq_along(Sleep))

trans_df <- tmp %>%
  select(id, TetanusVaccinated) %>%
  filter(!is.na(TetanusVaccinated)) %>%
  unique() %>%
  split(.$id) %>%
  lapply(function(x) as(x$TetanusVaccinated, "transactions")) %>%
  do.call(c, .)
```
...
```{r}
trans_df <- as(read.csv(here("data", "heart_data.csv")), "transactions")
```
