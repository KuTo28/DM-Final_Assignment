...
df <- df |>
  mutate(
    Sleep = case_when(
      Sleep %in% c(1:5, 11:24) ~ "Not Appropiated",
      Sleep %in% c(6, 10) ~ "Appropiated",
      Sleep %in% c(7:9) ~ "Optimal",
      TRUE ~ Slepp
    )
  )
```
...
```{r kmeans clustering}
MentalDistress <- data.frame(df$MentalHealthDays) |>
  na.omit() %>%
  sample_n(1000) |>
  scale()
...
hclust_metrics <- bind_rows(
  pmap(parameters, ~ perform_hclust(MentalDistress, ..1, num_clusters = 3))
)

(best_result <- hclust_metrics[which.max(hclust_metrics$silhouette), ])

plot_hclust(MentalDistress, best_result$method, best_result$num_clusters)
```

```{r}
# kmeans_result <- kmeans(MentalDistress, centers = 3)
#
# # Add cluster assignments to the original data frame
# MentalDistress$cluster <- as.factor(kmeans_result$cluster)
#
# ggplot(MentalDistress, aes(x = MentalDistress, y = as.numeric(cluster), color = cluster)) +
#   geom_point() +
#   labs(title = "K-means Clustering Results for Discrete Variable",
#        x = "Discrete Variable",
#        y = "Cluster") +
#   scale_color_manual(values = c("1" = "red", "2" = "green", "3" = "blue")) +  # Adjust colors as needed
#   theme_minimal()
```
