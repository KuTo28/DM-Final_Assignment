...
mhd_labels <- make_hclust(
  ...
  mutate(Cluster_labels = case_when(
    Cluster_labels == 1 ~ "Low",
    Cluster_labels == 2 ~ "High",
    Cluster_labels == 3 ~ "Medium"
  ))
```
...
```{r}
mhd_summary$MaxValue[c(1, 3)] <- c(9, 24)
```
...
phd_summary$MaxValue[c(1, 3)] <- c(12, 27)
...
df <- df |>
  mutate(
    HasSmoked = ifelse(
      SmokerStatus == "No" | ECigaretteUsage == "No",
      "No",
      "Yes"
    )
  )
...
```{r}
run_rule_induction <- function(df, support_values, min_confidence_values, target) {
  result_df <- data.frame()
...
results <- run_rule_induction(trans_df, support_values, min_confidence, c("TetanusVaccinated=1", "TetanusVaccinated=0"))
print(results)

results <- run_rule_induction(trans_df, support_values, min_confidence, c("PneumoVaxEver=1", "PneumoVaxEver=0"))
print(results)

results <- run_rule_induction(trans_df, support_values, min_confidence, c("HIVTesting=1", "HIVTesting=0"))
print(results)

results <- run_rule_induction(trans_df, support_values, min_confidence, c("ChestScan=1", "ChestScan=0"))
print(results)
```
