...
Finally, we can see the distribution of age between the data of the dataset:

```{r plot age_dist pie}
plot_pie_freq(df$AgeCategory, "Age distribution")
```

We can see that there is approximately a `63%` of people older than 50.
...
In the case of `RaceEthnicityCategory`, we decided to include the NA's, about 10k observations, in the `Multiracial` category. This decision aims to help balance the classes, as this category currently has 8k entries.
...
For `RemovedTeeth` we have supossed that if we don't know if any teeth has been removed `None` of them have been removed.
...
Mental and Physical health days exhibit values from 0-30. Preliminary observation suggests the existence of three distinct clusters within these variables. To objectively validate this intuition, we'll apply hierarchical clusterization. The next step involves the assignment of semantic labels:
...
Due to the small quantity of values, approximately 3000 'Not Known', we will add these values to the class with fewer data points, in this case, 'Yes', as illustrated in the pie chart below.
...
ggplot(frequency_results, aes(x = name, y = percentage, fill = result)) +
  ...
  labs(
    title = "Entropy per Model and Category",
    x = "Model", y = "Entropy"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
