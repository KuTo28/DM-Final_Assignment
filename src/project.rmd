...
!TO_FORMAL decidimos meter los NA's (10k) en mult
racial (ya que no sabemos de que raza es decimos que tiene un poco de cada xd)
para balancear las clases ya que este solo tiene 8k
```{r}
df <- df |>
  mutate(
...
!TO_FORMAL Given that, We'll crete a new column categorizing the values
...
!TO_FORMAL We'll do the same for `PhysicalHealthDays`
```{r hclust physical}
set.seed(29)
...
!TO_FORMAL DEbido a la poca cantidad de valores (3000 not knowns overall), los metermos a la clase que tenga menos datos. En este caso "yes"
```{r smoking_map}
df <- df |>
  mutate(
    HasSmoked = case_when(
...
ggplot(df_encoded, aes(x = as.factor(BMI), color = Gender)) +
  geom_jitter(aes(y = as.numeric(PhysicalHealthIndex)), width = 0.1) + # TODO: Check if PhysicalHealthIndex instead of PhysicalHealthDays
...
```{r}
trans_df <- df
columnas_numericas <- sapply(trans_df, is.numeric)
trans_df[columnas_numericas] <- lapply(trans_df[columnas_numericas], as.factor)
trans_df <- as(na.omit(trans_df), "transactions")
```

```{r}
inspect(trans_df[1:2, ])
summary(trans_df)
str(trans_df)

image(sample(trans_df, 100))
```
...
```{r}
run_apriori_with_loop <- function(df, supp_values, conf_values, minlen_values, target, max_rules = 5) {
  ...
  for (supp_val in supp_values) {
    for (conf_val in conf_values) {
      for (minlen_val in minlen_values) {
        ...
        reglas <- not_red_rules
        info_reglas <- inspect(reglas)
        df_reglas <- as.data.frame(info_reglas)
        is_vec <- is.vector(df_reglas)
        if (!is_vec) { next }
        df_reglas <- df_reglas[order(df_reglas$confidence, df_reglas$support), ]
...
supp_values <- c(0.4, 0.5)
conf_values <- c(0.7, 0.8, 0.9)
minlen_values <- c(1, 2, 3, 4)
```



```{r}
TetanusVaccinated_rules <- run_apriori_with_loop(
  trans_df,
  supp_values,
  conf_values,
  minlen_values,
  c("TetanusVaccinated=1", "TetanusVaccinated=0")
)
TetanusVaccinated_rules
```

```{r}
PneumoVaxEver_rules <- run_apriori_with_loop(
 trans_df,
 supp_values,
 conf_values,
 minlen_values,
 c("PneumoVaxEver=1", "PneumoVaxEver=0")
)
PneumoVaxEver_rules
```
...
```{r}
HIVTesting_rules <- run_apriori_with_loop(
 trans_df,
 supp_values,
 conf_values,
 minlen_values,
 c("HIVTesting=1", "HIVTesting=0")
)
HIVTesting_rules
```
...
```{r}
ChestScan_rules <- run_apriori_with_loop(
 trans_df,
 supp_values,
 conf_values,
 minlen_values,
 c("ChestScan=1", "ChestScan=0")
)
ChestScan_rules
```

```{r}
order_df_by <- function(data, min_conf = 0.7, desc = TRUE) {
  tmp <- data[data$Confidence > min_conf, ]
  tmp <- tmp[order(tmp$Confidence, tmp$Support, tmp$MinLength, decreasing = desc), ]
  return(tmp)
}
# TetanusVaccinated_rules
tmp_1 <- order_df_by(TetanusVaccinated_rules)
tmp_1[1, ]
# PneumoVaxEver_rules
tmp_2 <- order_df_by(PneumoVaxEver_rules)
tmp_2[1, ]
# HIVTesting_rules
tmp_3 <- order_df_by(HIVTesting_rules)
tmp_3[1, ]
# ChestScan_rules
tmp_4 <- order_df_by(ChestScan_rules)
tmp_4[1, ]
```

## Predictions for NA using Asoc rules

```{r}
newdata_df <- which(is.na(df$TetanusVaccinated))
rules <- apriori(
  trans_df,
  parameter = list(
    supp = 0.4,
  ...
  appearance = list(),
  appearance$rules <- subset(appearance, rhs %pin% "TetanusVaccinated")
)
res_inferidos <- predict(rules, newdata = newdata_df)
```
```{r}
missing_rows <- which(is.na(df$TetanusVaccinated))
...
rules_with_tetanus <- subset(rules, subset = rhs %pin% "TetanusVaccinated")

for (i in 1:length(rules_with_tetanus)) {
  rule <- rules_with_tetanus[[i]]
  antecedent <- lhs(rule)

  # Find matching rows with missing values and apply predictions
  matching_rows <- intersect(missing_rows,
                             which(apply(df[missing_rows, antecedent], 1, function(row) all(row == antecedent))))

  if (length(matching_rows) > 0) {
    prediction <- as(rule, "data.frame")[1, "TetanusVaccinated"]
    df$TetanusVaccinated[matching_rows] <- prediction
  }
}
```
