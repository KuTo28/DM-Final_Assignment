...
Tenemos que calcular los whiskers para luego clappear los outliers
```{r}
whisker_values <- df %>%
  group_by(Group) %>%
  summarize(
    lower_whisker = quantile(Value, 0.25) - 1.5 * IQR(Value),
    upper_whisker = quantile(Value, 0.75) + 1.5 * IQR(Value)
  )
```
